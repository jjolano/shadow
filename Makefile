...
ifeq ($(THEOS_PACKAGE_SCHEME),rootless)
	...
	$(ECHO_NOTHING)mv "$(THEOS_STAGING_DIR)/Library/LaunchDaemons/me.jjolano.shadow.watcher.rootless.plist" "$(THEOS_STAGING_DIR)/Library/LaunchDaemons/me.jjolano.shadow.watcher.plist"$(ECHO_END)
else
ifeq ($(findstring arm64,$(ARCHS)),)
# Legacy pass: no daemon binary ships, so no daemon plists either (launchd
# would crash-loop trying to exec a missing arm64 binary).
	...
	$(ECHO_NOTHING)rm -f "$(THEOS_STAGING_DIR)/Library/LaunchDaemons/me.jjolano.shadow.watcher.plist" "$(THEOS_STAGING_DIR)/Library/LaunchDaemons/me.jjolano.shadow.watcher.rootless.plist"$(ECHO_END)
else
