...
static NSString *shdw_identity_pointer_string(const void *pointer) {
  return [NSString
      stringWithFormat:@"0x%llx", (unsigned long long)(uintptr_t)pointer];
}

static NSDictionary<NSString *, id> *
shdw_identity_image_for_address(const void *address) {
  Dl_info info = {0};
  BOOL resolved = dladdr(address, &info) != 0;
  const char *imagePath = resolved ? info.dli_fname : NULL;
  const struct mach_header *header =
      resolved ? (const struct mach_header *)info.dli_fbase : NULL;
  ...
  return @{
    ...
    @"canonical_runtime" :
        @(imagePath && shdw_is_shadow_runtime_image(imagePath)),
  };
}
...
+ (NSDictionary<NSString *, id> *)
    shdw_identitySnapshotForBundleID:(NSString *)bundleID
                              scheme:(NSString *)scheme;
+ (NSDictionary<NSString *, id> *)shdw_identityImageForAddress:
    (NSValue *)address;
- (NSDictionary<NSString *, id> *)shdw_activationSnapshot;
...
- (instancetype)initWithInstallerTable:(const SHDWPluginInstaller *)installers
  ...
  _lifecycleQueue = dispatch_queue_create(
      "com.shadow.hookcoordinator.lifecycle", DISPATCH_QUEUE_SERIAL);
  ...
  set.capabilities =
      SHDWCapMessage | SHDWCapFunction | SHDWCapInline | SHDWCapPrivateSym;
  self.backends = set;
...
- (NSDictionary<NSString *, id> *)shdw_activationSnapshot {
  ...
  if (dispatch_get_specific(&kSHDWHookCoordinatorQueueKey) ==
      (__bridge void *)self) {
    copyState();
  } else {
...
+ (NSDictionary<NSString *, id> *)
  ...
  SHADOW_INTERNAL_SCOPE {
    ...
    (void)dlopen("/System/Library/Frameworks/MobileCoreServices.framework/"
                 "MobileCoreServices",
    ...
    if (bundleID && proxyClass &&
        class_getClassMethod(proxyClass, proxySelector)) {
      typedef id (*ProxyMessage)(id, SEL, id);
      proxySupported = YES;
      proxyPresent = ((ProxyMessage)objc_msgSend)((id)proxyClass, proxySelector,
                                                  bundleID) != nil;
    }
    ...
    SEL schemeSelector =
        sel_registerName("applicationsAvailableForHandlingURLScheme:");

    if (scheme && workspaceClass &&
        class_getClassMethod(workspaceClass, workspaceSelector)) {
      typedef id (*WorkspaceMessage)(id, SEL);
      id workspace = ((WorkspaceMessage)objc_msgSend)((id)workspaceClass,
                                                      workspaceSelector);

      if (workspace && [workspace respondsToSelector:schemeSelector]) {
        typedef id (*SchemeMessage)(id, SEL, id);
        id values =
            ((SchemeMessage)objc_msgSend)(workspace, schemeSelector, scheme);
        schemeSupported = YES;
    ...
    snapshot = @{
      @"identity" :
          shdw_identity_image_for_address((const void *)implementation),
      @"filesystem" : @{@"result" : @(statResult), @"errno" : @(statErrno)},
      @"dyld" : @{
        @"image_count" : @(imageCount),
        @"runtime_image_count" : @(runtimeImages)
      },
...
- (NSUInteger)unitIndexForID:(NSString *)unitID {
  ...
  for (NSUInteger i = 0; i < count; i++) {
    if ([[NSString stringWithUTF8String:plugins[i].unitID]
            isEqualToString:unitID]) {
      return i;
    }
  }
...
- (NSUInteger)installEvent:(SHDWLifecycleEvent)event {
  if (event < SHDWEventCtor || event > SHDWEventSDKFallback)
    return 0;
  ...
  if (dispatch_get_specific(&kSHDWHookCoordinatorQueueKey) ==
    ...
    dispatch_sync(self.lifecycleQueue, work);
  if (failure)
    @throw failure;
  return installed;
}
...
+ (void)shdw_requestPendingTargetDrain {
  ...
  if (__atomic_exchange_n(&requested, YES, __ATOMIC_ACQ_REL))
    return;
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    dispatch_sync(coordinator.lifecycleQueue, ^{
      ...
      if (failure)
        NSLog(@"[Shadow] pending target drain failed: %@", failure);
    });
  });
}
...
- (NSUInteger)installEventSync:(SHDWLifecycleEvent)event {
  ...
  NSArray<NSString *> *plan =
      SHDWPluginPlan(self.prefs, self.backends.capabilities, event);
...
- (BOOL)installHarnessSDKFallback {
  if (!_harnessProfile ||
      __atomic_exchange_n(&_sdkFallbackInstalled, YES, __ATOMIC_ACQ_REL)) {
    return NO;
  }
