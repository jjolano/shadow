...
// packaged builds. Resolve it dynamically so absence cleanly keeps auto
// routing.
...
static void shdw_note_published_cell(void **cell) {
  ...
  if (count == SHDW_PUBLISHED_CELLS_MAX)
    return;
...
static void *shdw_prepared_original(hk_hook_t *hook, const hk_hook_spec_t *spec,
...
// amIRuntimeHooked). Record replacement->original at hook time so
// replaced_dladdr can answer a query about a replacement IMP as though it were
// the original IMP, which still lives in the genuine system framework. Only
// entries whose original is a real (non-Shadow) image are useful; the dladdr
// path re-validates.
typedef struct {
...
void SHDWRememberHookedIMPRemap(const void *replacement, const void *original) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    if (gSHDWHookedIMPRemaps[i].replacement == (uintptr_t)replacement)
      return;
  }
  if (count == sizeof(gSHDWHookedIMPRemaps) / sizeof(gSHDWHookedIMPRemaps[0]))
    return;
  gSHDWHookedIMPRemaps[count] =
      (SHDWHookedIMPRemap){(uintptr_t)replacement, original};
...
static void SHDWRememberImportSlot(uintptr_t start, size_t size,
  ...
  uint32_t overflowedBefore =
      __atomic_load_n(&gSHDWRebindSlots.overflowed, __ATOMIC_ACQUIRE);
  ...
  __atomic_store_n(&gSHDWRebindSlots.count, gSHDWRebindSlots.count,
                   __ATOMIC_RELEASE);
  ...
  if (!overflowedBefore &&
      __atomic_load_n(&gSHDWRebindSlots.overflowed, __ATOMIC_ACQUIRE)) {
    NSLog(
        @"[Shadow] rebind journal full (%u slots) — further slots unguarded+unrepaired",
        __atomic_load_n(&gSHDWRebindSlots.count, __ATOMIC_ACQUIRE));
  }
}

static void SHDWRememberImportSlots(hk_report_t *report,
                                    uintptr_t fallbackExpected) {
  hk_artifact_snapshot_t *snapshot = NULL;
  if (!report || hk_report_copy_artifacts(report, &snapshot) != HK_STATUS_OK ||
      !snapshot)
    return;
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (hk_artifact_snapshot_copy_at(snapshot, i, &artifact) == HK_STATUS_OK &&
      ...
      uintptr_t expected = artifact.replacement_pointer
                               ? (uintptr_t)artifact.replacement_pointer
                               : fallbackExpected;
...
BOOL SHDWRangeOverlapsProtectedImportSlots(uintptr_t address, size_t size) {
  if (!address || !size)
    return NO;
  uint32_t count = __atomic_load_n(&gSHDWRebindSlots.count, __ATOMIC_ACQUIRE);
  return shdw_rebind_slots_overlap(&gSHDWRebindSlots, count, address, size)
             ? YES
             : NO;
}
...
static void SHDWRememberOriginalImplementation(Method method, IMP original) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    if (gSHDWOriginalIMPs[i].method == method)
      return;
  }
  if (count == sizeof(gSHDWOriginalIMPs) / sizeof(gSHDWOriginalIMPs[0]))
    return;
...
IMP SHDWOriginalImplementationForMethod(Method method) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    if (gSHDWOriginalIMPs[i].method == method)
      return gSHDWOriginalIMPs[i].original;
  }
  return NULL;
}

SHDWHookSession *SHDWHookSessionSetCurrent(SHDWHookSession *session) {
  SHDWHookSession *previous =
      (__bridge SHDWHookSession *)gSHDWCurrentHookSession;
...
static BOOL shdw_apply_hook_spec_once(
  ...
  hk_status_t runtimeStatus =
      backendOverride ? createWithOverride(&config, backendOverride, &runtime)
...
static BOOL shdw_apply_hook_spec(const hk_hook_spec_t *spec, void **oldPtr) {
  ...
  BOOL installed = shdw_apply_hook_spec_once(spec, oldPtr, backendOverride,
                                             createWithOverride, &cleanRefusal);

  if (!installed && backendOverride && cleanRefusal &&
      spec->target_kind != HK_TARGET_OBJC_METHOD) {
    installed =
        shdw_apply_hook_spec_once(spec, oldPtr, NULL, createWithOverride, NULL);
  }
  return installed;
}
...
- (instancetype)init {
  return [self initWithLifecycleQueue:dispatch_queue_create(
                                          "com.shadow.hooksession.lifecycle",
...
- (instancetype)initWithLifecycleQueue:(dispatch_queue_t)queue {
  ...
  if (self) {
    ...
    dispatch_queue_set_specific(queue, (__bridge const void *)self,
...
- (void)dealloc {
  if (_lifecycleQueue)
    dispatch_queue_set_specific(_lifecycleQueue, (__bridge const void *)self,
                                NULL, NULL);
}
...
- (BOOL)performWhenTargetAvailable:(BOOL (^)(SHDWHookSession *))attempt {
  ...
  void (^work)(void) = ^{
    @try {
      ...
      if (!completed)
        [_pendingTargets addObject:[attempt copy]];
  ...
  if (dispatch_get_specific((__bridge const void *)self))
    ...
    dispatch_sync(_lifecycleQueue, work);
  if (failure)
    @throw failure;
  return completed;
}
  ...
  void (^work)(void) = ^{
    ...
    @try {
      for (BOOL (^attempt)(SHDWHookSession *) in [_pendingTargets copy]) {
        ...
        @try {
          if (!attempt(self))
            [_pendingTargets addObject:attempt];
        } @catch (NSException *exception) {
          if (!failure)
            failure = exception;
        }
...
- (BOOL)hookMessageInClass:(Class)objcClass
  ...
  shdw_init_spec(&spec, "shadow.objc", HK_TARGET_OBJC_METHOD, replacement,
                 ...
                 oldPtr ? HK_ORIGINAL_DIRECT_PREDECESSOR : HK_ORIGINAL_NONE);
  ...
  if (installed && oldPtr && *oldPtr) {
    ...
    if (methodAfter) {
      SHDWRememberHookedIMPRemap(
          (const void *)method_getImplementation(methodAfter), *oldPtr);
    }
...
- (BOOL)hookFunction:(void *)function
  ...
  shdw_init_spec(&spec, "shadow.function", HK_TARGET_FUNCTION_ADDRESS,
                 replacement, HK_REACH_ENTRYPOINT,
                 oldPtr ? HK_ORIGINAL_CALLABLE_CONTINUATION : HK_ORIGINAL_NONE);
...
- (BOOL)hookRebindSymbol:(NSString *)symbolName
  ...
  hk_hook_spec_t spec;
  shdw_init_spec(&spec, "shadow.rebind", HK_TARGET_FUNCTION_SYMBOL, replacement,
  ...
  spec.target.symbol.name_convention = [symbolName hasPrefix:@"$s"]
                                           ...
                                           : HK_SYMBOL_NAME_C;
  spec.target.symbol.defining_image.struct_size =
      sizeof(spec.target.symbol.defining_image);
  ...
  spec.target.symbol.caller_image_scope.struct_size =
      sizeof(spec.target.symbol.caller_image_scope);
  spec.target.symbol.caller_image_scope.struct_version = HK_ABI_VERSION_3_0;
  spec.target.symbol.caller_image_scope.kind =
      imageHeader ? HK_IMAGE_EXACT_HEADER : HK_IMAGE_ANY_LOADED;
...
- (void *)findSymbolInImage:(SHDWImageRef)image
