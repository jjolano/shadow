  ...
  @autoreleasepool {
    ...
    if ([bundleID hasPrefix:@"me.jjolano.shadow.test."] &&
        shdw_path_is_within(absolute,
                            @"/var/mobile/Documents/ShadowDetectorTests"))
      return NO;
    if (shdw_path_is_within(absolute, shadow.bundlePath))
      return YES;
    if (shdw_path_is_within(absolute, shadow.homePath))
      return NO;
    if (shdw_path_is_within(absolute,
                            @"/var/mobile/Containers/Shared/AppGroup"))
      return NO;
    if ([absolute isEqualToString:@"/dev/null"])
      return NO;
    return YES;
  }
}
...
BOOL shdw_detector_c_write_path_denied(const char *path) {
  return path &&
         shdw_detector_write_path_denied([NSString stringWithUTF8String:path]);
}
...
BOOL shdw_detector_c_write_path_at_denied(int dirfd, const char *path) {
  if (!path || !path[0])
    return NO;
  if (path[0] == '/')
    return shdw_detector_c_write_path_denied(path);
  int saved_errno = errno;
  char parent[PATH_MAX];
  BOOL denied = NO;
  if (shdw_resolve_dirfd_path(dirfd, path, parent, sizeof(parent)) ==
      SHADW_DIRFD_OK) {
...
// ponytail: jb-root prefixes centralized via JBPath
// shdw_path_contains_restricted_root_c (single source)
...
static BOOL shdw_path_is_jb_app_container(const char *pathname) {
  ...
  if (strncmp(pathname, "/var/mobile/Library/", 20) == 0)
    rest = pathname + 20;
  else if (strncmp(pathname, "/var/root/Library/", 18) == 0)
    rest = pathname + 18;
  if (!rest)
    return NO;
  ...
  for (int i = 0; jbAppIDs[i]; i++) {
    ...
    if (strncmp(leaf, jbAppIDs[i], n) == 0)
      return YES;
  }
  return NO;
}
...
const char *shdw_standardize_lexical(const char *path) {
  ...
  if (len >= sizeof(shdw_lex_scratch))
    return path;
  ...
  while (i <= len) {
    ...
    if (clen == 0) {
      ...
      if (depth >= sizeof(starts) / sizeof(starts[0]))
        return path;
      starts[depth++] = w;
      if (w + clen + 2 > sizeof(shdw_lex_scratch))
        return path;
...
BOOL shdw_path_is_external_hidden_lexical(const char *path) {
  ...
  for (int i = 0; paths[i]; i++) {
    if (strlen(paths[i]) == n && strncmp(std, paths[i], n) == 0)
      return YES;
  }
...
BOOL shdw_path_is_external_hidden(const char *pathname) {
  ...
  if (shdw_path_is_external_hidden_lexical(pathname))
    return YES;
...
BOOL shdw_path_is_external_hidden_nofollow(const char *pathname) {
  ...
  if (physical && shdw_path_is_external_hidden_lexical(physical))
    return YES;
  return NO;
}
...
BOOL shdw_path_ruleset_denied_nofollow(const char *path) {
  ...
  BOOL denied =
      (p != nil) && [_shadow isPathRestricted:p
                                      options:@{
...
BOOL shdw_dir_leaf_external_hidden(const char *parent, const char *d_name) {
  ...
  if (shdw_path_under_system_bind_root(parent))
    return YES;
  if (strstr(parent, ".fakelib") != NULL)
    return YES;
  if (strstr(parent, "/procursus/basebin") != NULL)
    return YES;
  return NO;
}
...
BOOL shdw_resolved_spelling_hidden(const char *canon) {
  if (!canon || canon[0] == '\0')
    return NO;
  if (shdw_path_is_external_hidden(canon))
    return YES;
  const char *slash = strrchr(canon, '/');
  if (!slash || slash == canon || slash[1] == '\0')
    return NO;
  char parent[PATH_MAX];
...
BOOL shdw_path_under_system_bind_root(const char *pathname) {
  ...
  static const char *const roots[] = {"/usr/lib", "/usr/libexec", "/System",
                                      NULL};
  for (int i = 0; roots[i]; i++) {
    size_t n = strlen(roots[i]);
    if (strncmp(pathname, roots[i], n) == 0 &&
        (pathname[n] == '\0' || pathname[n] == '/')) {
      return YES;
    }
  }
  return NO;
...
BOOL shdw_path_is_main_bundle_exempt(const char *pathname) {
  ...
  dispatch_once(&once, ^{
    ...
    if (!cachedBundle)
      cachedBundle = [[[NSBundle mainBundle] bundlePath] copy];
    if (!cachedExe)
      cachedExe = [[[NSBundle mainBundle] executablePath] copy];
  });
  if (!cachedBundle && !cachedExe)
    return NO;
  @autoreleasepool {
    NSString *p = [NSString stringWithUTF8String:pathname];
    if (!p)
      return NO;
    if (cachedExe && [p isEqualToString:cachedExe])
      return YES;
    if (cachedBundle &&
        ([p isEqualToString:cachedBundle] ||
         [p hasPrefix:[cachedBundle stringByAppendingString:@"/"]]))
      return YES;
    ...
    const char *physical = shdw_path_physical_spelling(raw);
    if (physical && shdw_path_is_main_bundle_exempt(physical))
      return YES;
    return NO;
  }
}
...
static BOOL shdw_path_under_immutable_prefix(const char *path) {
  ...
  static const char *const roots[] = {
      "/usr/",     "/System/",       "/bin/",
      "/sbin/",    "/etc/",          "/private/etc/",
      "/Library/", "/Applications/", "/Developer/",
      "/cores/",   "/dev/",          "/private/preboot/",
      "/preboot/", "/var/jb/",       "/private/var/jb/",
      NULL,
  };
  for (int i = 0; roots[i]; i++) {
    size_t n = strlen(roots[i]);
    if (strncmp(path, roots[i], n) == 0)
      return YES;
  }
  return NO;
}
...
// Shared shape of both twins below (see the header contract): re-open the
// request spelling and classify the re-opened object with the same resolving
// shape as the substitution verifier. With match_identity set, a re-opened
// object that is not the answered one (the atomic-swap case) denies as
// contradictory instead of admitting.
static shdw_post_verdict_t
shdw_at_post_verify_impl(int dirfd, const char *pathname, uint64_t answered_dev,
                         uint64_t answered_ino, BOOL match_identity) {
  if (!pathname || !pathname[0])
    return SHDW_POST_ADMIT;
  int saved_errno = errno;
  ...
  if (fd < 0) {
    ...
    verdict =
        (errno == ENOENT) ? SHDW_POST_DENY_CONTRADICTION : SHDW_POST_ADMIT;
  } else {
    char canon[PATH_MAX];
    struct stat sb;
    ...
    if (fcntl(fd, F_GETPATH, canon) != -1 && fstat(fd, &sb) == 0) {
      ...
      } else if (match_identity && ((uint64_t)sb.st_dev != answered_dev ||
...
shdw_post_verdict_t shdw_at_post_verify(int dirfd, const char *pathname) {
  return shdw_at_post_verify_impl(dirfd, pathname, 0, 0, NO);
}
...
shdw_post_verdict_t shdw_at_post_verify_match(int dirfd, const char *pathname,
  ...
  return shdw_at_post_verify_impl(dirfd, pathname, answered_dev, answered_ino,
                                  YES);
}
BOOL shdw_at_post_hidden(int dirfd, const char *pathname) {
  if (!pathname || !pathname[0])
    return NO;
  if (pathname[0] == '/')
    return shdw_path_post_hidden(pathname);
  int saved_errno = errno;
  BOOL hidden = NO;
  char parent[PATH_MAX];
  if (shdw_resolve_dirfd_path(dirfd, pathname, parent, sizeof(parent)) ==
      SHADW_DIRFD_OK) {
    char joined[PATH_MAX * 2];
...
// the documented public ABI; the path field's offset is all that is
// read/written.
struct shdw_vinfo_stat {
  ...
  uint32_t vst_uid;
  uint32_t vst_gid;
  int64_t vst_atime, vst_atimensec, vst_mtime, vst_mtimensec, vst_ctime,
      vst_ctimensec;
...
struct shdw_vnode_info {
  struct shdw_vinfo_stat vi_stat;
  int vi_type;
  int vi_pad;
  uint32_t vi_fsid[2];
};
struct shdw_vnode_info_path {
  struct shdw_vnode_info vip_vi;
  char vip_path[PATH_MAX];
};
...
void shdw_region_path_result_sanitize(int flavor, void *buffer,
                                      int buffersize) {
  ...
  if (flavor == SHADOW_PROC_PIDREGIONPATHINFO ||
    ...
    if ((size_t)buffersize < sizeof(struct shdw_proc_regionwithpathinfo))
      return;
    pathField =
        ((struct shdw_proc_regionwithpathinfo *)buffer)->prp_vip.vip_path;
  } else if (flavor == SHADOW_PROC_PIDREGIONPATH) {
    if ((size_t)buffersize < sizeof(struct shdw_proc_regionpath))
      return;
...
static BOOL shdw_vnodepath_hidden(const char *path) {
  ...
  @autoreleasepool {
    ...
    NSString *bundlePath = [[NSBundle mainBundle] bundlePath];
    if (bundlePath &&
        ([@(path) isEqualToString:bundlePath] ||
...
void shdw_vnodepath_result_sanitize(void *buffer, int buffersize) {
  if (!buffer || (size_t)buffersize < sizeof(struct shdw_proc_vnodepathinfo))
    return;

  struct shdw_proc_vnodepathinfo *info =
      (struct shdw_proc_vnodepathinfo *)buffer;
  char *fields[2] = {info->pvi_cdir.vip_path, info->pvi_rdir.vip_path};

  for (int i = 0; i < 2; i++) {
    char *vip_path = fields[i];
    vip_path[PATH_MAX - 1] = '\0';
    if (!shdw_vnodepath_hidden(vip_path))
      continue;
...
shdw_dirfd_status_t shdw_resolve_dirfd_path(int dirfd, const char *path,
  ...
  if (dirfd == AT_FDCWD) {
    shdw_dirfd_status_t status =
        getcwd(out, outlen) ? SHADW_DIRFD_OK : SHADW_DIRFD_DENY;
    errno = saved_errno;
    return status;
  }
...
BOOL shdw_at_path_denied(int dirfd, const char *pathname) {
  ...
  char parent[PATH_MAX];
  shdw_dirfd_status_t status =
      shdw_resolve_dirfd_path(dirfd, pathname, parent, sizeof(parent));
  ...
  if (status == SHADW_DIRFD_ABSOLUTE) {
    ...
    if (shdw_path_is_external_hidden(pathname) ||
    ...
    BOOL restricted =
        [_shadow isPathRestricted:path
                          options:@{
                            kShadowRestrictionWorkingDir :
                                [NSString stringWithUTF8String:parent]
                          }];
...
BOOL shdw_at_path_denied_nofollow(int dirfd, const char *pathname) {
  ...
  if (status == SHADW_DIRFD_ABSOLUTE) {
    ...
    if (shdw_path_is_external_hidden_nofollow(pathname) ||
        shdw_path_ruleset_denied_nofollow(pathname)) {
      errno = ENOENT;
      return YES;
    }
  } else if (status == SHADW_DIRFD_DENY) {
    ...
    int n = snprintf(joined, sizeof(joined), "%s/%s", parent, pathname);
    if ((n > 0 && n < (int)sizeof(joined) &&
         shdw_path_is_external_hidden_nofollow(joined)) ||
        shdw_dir_leaf_external_hidden(parent, pathname)) {
      errno = ENOENT;
      return YES;
    }
...
NSDictionary *shdw_readdir_options(DIR *dirp, BOOL *denied) {
  ...
  *denied = NO;
  char pathname[PATH_MAX];
  shdw_dirfd_status_t status =
      shdw_resolve_dirfd_path(dirfd(dirp), ".", pathname, sizeof(pathname));

  if (status == SHADW_DIRFD_OK) {
    NSDictionary *options = @{
      kShadowRestrictionWorkingDir : [NSString stringWithUTF8String:pathname]
    };
    errno = saved_errno;
...
BOOL shdw_readlink_target_restricted(int dirfd, const char *pathname,
  ...
  if (pathname[0] == '/') {
    ...
    linkPath = [[NSString stringWithUTF8String:parent]
        stringByAppendingPathComponent:[NSString
                                           stringWithUTF8String:pathname]];
  }
