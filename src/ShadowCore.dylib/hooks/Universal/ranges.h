...
static inline int
shdw_is_canonical_shadow_runtime_path_under_root(const char *path,
                                                 const char *root) {
...
static inline shdw_range_verdict_t
shdw_ranges_lookup(const shdw_restricted_ranges_t *table, uint64_t generation,
                   uintptr_t addr) {
  if (!table) {
