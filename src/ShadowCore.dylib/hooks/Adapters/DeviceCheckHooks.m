...
static NSError *shdw_dch_unsupportedError(BOOL isAppAttestKey) {
  ...
  return [NSError errorWithDomain:resolvedDomain
                             code:1
...
static void shdw_dch_imp1_forge_unsupported(id self, SEL _cmd, id completion) {
  ...
  if (!completion)
    return;
  BOOL keyFlow =
      sel_isEqual(_cmd, sel_registerName("generateKeyWithCompletionHandler:"));
...
static void shdw_dch_imp3_forge_unsupported(id self, SEL _cmd, id a0, id a1,
                                            id completion) {
  (void)self;
  (void)_cmd;
  (void)a0;
  ...
  if (!completion)
    return;
  NSError *err =
      shdw_dch_unsupportedError(YES); // attest/assert are AppAttest key flows.
  void (^block)(id, id) = completion;
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
    block(nil, err);
  });
...
const DCHDescriptor shdw_devicecheck_descriptors[] = {
    {"UBReportMetadataDevice", "is_rooted", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"UBReportMetadataDevice", "is_rooted", DCHMethodInstance, '@', 0,
     DCHPolicyFalse},
    {"UBReportMetadataDevice", "is_rooted", DCHMethodInstance, '^', 0,
     DCHPolicyFalse},
    {"EnrollParameters", "jailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"EnrollParameters", "jailbroken", DCHMethodInstance, '@', 0,
     DCHPolicyFalse},
    {"EnrollParameters", "jailbroken", DCHMethodInstance, '^', 0,
     DCHPolicyFalse},
    ...
    {"DCAppAttestService", "isSupported", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    ...
    {"DCDevice", "generateTokenWithCompletionHandler:", DCHMethodInstance, 'v',
     1, DCHPolicyForgeUnsupported},
    {"DCAppAttestService", "generateKeyWithCompletionHandler:",
     DCHMethodInstance, 'v', 1, DCHPolicyForgeUnsupported},
    {"DCAppAttestService", "attestKey:clientDataHash:completionHandler:",
     DCHMethodInstance, 'v', 3, DCHPolicyForgeUnsupported},
    {"DCAppAttestService",
     "generateAssertion:clientDataHash:completionHandler:", DCHMethodInstance,
     'v', 3, DCHPolicyForgeUnsupported},
    ...
    {"JailbreakDetectionVC", "isJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"DTTJailbreakDetection", "isJailbroken", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"SafeDeviceJailbreakDetection", "isJailbroken", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"SafeDevicePlugin", "isJailBroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"SafeDevicePlugin", "isJailBrokenCustom", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"SafeDevicePlugin", "hasObviousJailbreakSigns", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"JailMonkey", "isJailBroken", DCHMethodInstance, 'B', 0, DCHPolicyFalse},
    {"ANSMetadata", "computeIsJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    ...
    {"AppsFlyerUtils", "isJailbrokenWithSkipAdvancedJailbreakValidation:",
     DCHMethodClass, 'B', 1, DCHPolicyFalse},
    {"jailBreak", "isJailBreak", DCHMethodClass, 'B', 0, DCHPolicyFalse},
    {"GBDeviceInfo", "isJailbroken", DCHMethodInstance, 'B', 0, DCHPolicyFalse},
    {"CMARAppRestrictionsDelegate", "isDeviceNonCompliant", DCHMethodInstance,
     'B', 0, DCHPolicyFalse},
    {"ADYSecurityChecks", "isDeviceJailbroken", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"UtilitySystem", "isJailbreak", DCHMethodClass, 'B', 0, DCHPolicyFalse},
    {"GemaltoConfiguration", "isJailbreak", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},

    // Batch 3.
    {"CPWRDeviceInfo", "isJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"CPWRSessionInfo", "isJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"KSSystemInfo", "isJailbroken", DCHMethodClass, 'B', 0, DCHPolicyFalse},
    {"EMDSKPPConfiguration", "jailBroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"EMDskppConfigurationBuilder", "jailbreakStatus", DCHMethodInstance, 'B',
     0, DCHPolicyFalse},
    {"FCRSystemMetadata", "isJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},

    // Batch 4.
    {"v_VDMap", "isJailbrokenDetected", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"v_VDMap", "isJailBrokenDetectedByVOS", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"v_VDMap", "isDFPHookedDetecedByVOS", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"v_VDMap", "isCodeInjectionDetectedByVOS", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"v_VDMap", "isDebuggerCheckDetectedByVOS", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"v_VDMap", "isAppSignerCheckDetectedByVOS", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"v_VDMap", "v_checkAModified", DCHMethodInstance, 'B', 0, DCHPolicyFalse},
    {"v_VDMap", "isRuntimeTamperingDetected", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"SDMUtils", "isJailBroken", DCHMethodInstance, 'B', 0, DCHPolicyFalse},
    {"OneSignalJailbreakDetection", "isJailbroken", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"DigiPassHandler", "rootedDeviceTestResult", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"AWMyDeviceGeneralInfo", "isCompliant", DCHMethodInstance, 'B', 0,
     DCHPolicyTrue},

    // Batch 5.
    {"DTXSessionInfo", "isJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"DTXDeviceInfo", "isJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"JailbreakDetection", "jailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"jailBrokenJudge", "isJailBreak", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"jailBrokenJudge", "isCydiaJailBreak", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"jailBrokenJudge", "isApplicationsJailBreak", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"jailBrokenJudge", "ischeckCydiaJailBreak", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"jailBrokenJudge", "isPathJailBreak", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"jailBrokenJudge", "boolIsjailbreak", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"FBAdBotDetector", "isJailBrokenDevice", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},

    // Batch 6.
    {"TNGDeviceTool", "isJailBreak", DCHMethodClass, 'B', 0, DCHPolicyFalse},
    {"TNGDeviceTool", "isJailBreak_file", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"TNGDeviceTool", "isJailBreak_cydia", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"TNGDeviceTool", "isJailBreak_appList", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"TNGDeviceTool", "isJailBreak_env", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {"DTDeviceInfo", "isJailbreak", DCHMethodClass, 'B', 0, DCHPolicyFalse},
    {"SecVIDeviceUtil", "isJailbreak", DCHMethodClass, 'B', 0, DCHPolicyFalse},
    {"RVPBridgeExtension4Jailbroken", "isJailbroken", DCHMethodInstance, 'B', 0,
     DCHPolicyFalse},
    {"ZDetection", "isRootedOrJailbroken", DCHMethodClass, 'B', 0,
     DCHPolicyFalse},
    {NULL, NULL, 0, 0, 0, 0}};
...
static IMP shdw_dch_replacement_imp(const DCHDescriptor *desc) {
  ...
  if (desc->argCount == 0) {
    return truePolicy ? (IMP)&shdw_dch_imp0_bool_true
                      : (IMP)&shdw_dch_imp0_bool_false;
  }

  return truePolicy ? (IMP)&shdw_dch_imp1_bool_true
                    : (IMP)&shdw_dch_imp1_bool_false;
}

NSUInteger shdw_devicecheck_install_hooks(SHDWHookSession *hooks,
                                          DCHTarget enabledTargets) {
  if (!hooks) {
    return 0;
  }
  ...
  for (const DCHDescriptor *desc = shdw_devicecheck_descriptors;
       desc->className; desc++) {
    ...
    BOOL completed = [hooks performWhenTargetAvailable:^BOOL(
                                SHDWHookSession *session) {
      if (target != DCHTargetNone && !shdw_devicecheck_target_available(target))
        return NO;
      ...
      Class dispatchClass =
          desc->kind == DCHMethodClass ? object_getClass(cls) : cls;
      ...
      if (!encoding) {
        NSLog(@"[Shadow] DeviceCheck: skipping %s%s%s",
              desc->kind == DCHMethodClass ? "+" : "-", desc->className,
              desc->selector);
        return YES;
      }
      ...
      if (!rowMatches ||
          method_getNumberOfArguments(method) != desc->argCount + 2) {
        if (shdw_dch_encoding_is_unknown(e0)) {
          ...
          if (strcmp(s_loggedUnknown, key) != 0) {
            NSLog(@"[Shadow] DeviceCheck: skipping %s", key);
      ...
      if (desc->encoding == 'v') {
        for (unsigned int arg = 2; arg < desc->argCount + 2; arg++) {
          ...
          if (type[0] != '@' || (arg == desc->argCount + 1 && type[1] != '?'))
            return YES;
        }
      }

      succeeded =
          [session hookMessageInClass:dispatchClass
                         withSelector:sel
                      withReplacement:(void *)shdw_dch_replacement_imp(desc)
                            outOldPtr:NULL];
      return YES;
    }];
