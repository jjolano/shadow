...
typedef struct {
  ...
  char encoding;    // accepted return encoding: 'B', 'c'/'C', '@', '^', or 'v'
                    // (forge rows)
  uint8_t argCount; // args after self/_cmd: 0, 1, or 3 (forge rows use 1 or 3)
  DCHPolicy
      policy; // false/true for scalars; forge selector for 'v'; ignored for '@'
} DCHDescriptor;
...
static inline BOOL shdw_dch_imp1_bool_false(id self, SEL _cmd, id a0) {
  return NO;
}
static inline BOOL shdw_dch_imp1_bool_true(id self, SEL _cmd, id a0) {
  return YES;
}
