...
- (PSTableCell *)tableView:(UITableView *)tableView
  ...
  if (appID.length && !SHDWAppFollowsGlobal(prefs, appID)) {
    UIStackView *accessory =
        [[UIStackView alloc] initWithFrame:CGRectMake(0, 0, 42, 24)];
      ...
      if (image) {
        ...
        BOOL rtl =
            [UIView userInterfaceLayoutDirectionForSemanticContentAttribute:
                        cell.semanticContentAttribute] ==
            UIUserInterfaceLayoutDirectionRightToLeft;
        label.text = [name isEqualToString:@"slider.horizontal.3"]
                         ? @"\u2699"
                         : (rtl ? @"<" : @">");
    ...
    NSString *customized = [[NSBundle bundleForClass:[self class]]
        localizedStringForKey:@"CUSTOMIZED"
                        value:nil
                        table:@"App"];
    NSString *preview = [self previewStringForApplicationWithIdentifier:appID];
    NSString *format = [[NSBundle bundleForClass:[self class]]
        localizedStringForKey:@"APP_STATE_ACCESSIBILITY_FMT"
                        value:nil
                        table:@"App"];
    cell.accessibilityValue =
        preview.length ? [NSString stringWithFormat:format, preview, customized]
                       : customized;
  }
  return cell;
}
