...
- (NSString *)localized:(NSString *)key {
  return
      [[NSBundle bundleForClass:[self class]] localizedStringForKey:key
                                                              value:key
                                                              table:@"About"];
}
...
- (void)checkForUpdates:(id)sender {
  ...
  // Preserve the release-list selection, excluding artifact-only tags and
  // prereleases.
  NSURL *updateURL = [NSURL
      URLWithString:
          @"https://api.github.com/repos/jjolano/shadow/releases?per_page=10"];
  ...
  latestVersionTask = [releaseSession
        dataTaskWithURL:updateURL
          ...
          if ([json isKindOfClass:[NSArray class]]) {
            ...
            for (id release in (NSArray *)json) {
              if (![release isKindOfClass:[NSDictionary class]] ||
                  ...
                  [release[@"prerelease"] boolValue])
                continue;
              NSString *tag_name = release[@"tag_name"];
              if (![tag_name isKindOfClass:[NSString class]] ||
                  tag_name.length == 0)
                continue;
              NSString *candidate = [tag_name hasPrefix:@"v"]
                                        ? [tag_name substringFromIndex:1]
                                        : tag_name;
              if ([candidate rangeOfString:@"^[0-9]+\\.[0-9]+"
                                   options:NSRegularExpressionSearch]
                      .location != NSNotFound) {
                version = candidate;
                title = [release[@"name"] isKindOfClass:[NSString class]]
                            ? release[@"name"]
                            : nil;
                if (!title.length)
                  title = tag_name;
                body = [release[@"body"] isKindOfClass:[NSString class]]
                           ? release[@"body"]
                           : nil;
                // Only an unambiguous release page in this repository may be
                // opened.
                NSString *htmlURL = release[@"html_url"];
                if ([htmlURL isKindOfClass:[NSString class]]) {
                  NSURLComponents *components =
                      [NSURLComponents componentsWithString:htmlURL];
                  NSString *prefix = @"/jjolano/shadow/releases/tag/";
                  NSString *path = components.percentEncodedPath;
                  if ([components.scheme isEqualToString:@"https"] &&
                      [components.host isEqualToString:@"github.com"] &&
                      !components.port && !components.user &&
                      !components.password && !components.query &&
                      !components.fragment && [path hasPrefix:prefix] &&
                      [[path substringFromIndex:prefix.length]
                          isEqualToString:tag_name] &&
                      [tag_name
...
- (NSString *)updateStatus {
  if (releaseFetchFailed)
    return [self localized:@"NOTES_ERROR"];
  if (!latestVersion)
    return [self localized:@"NOTES_NO_RELEASE"];
  ...
  BOOL outdated = [installed compare:latestVersion
...
- (CGFloat)tableView:(UITableView *)tableView
  ...
  return indexPath.section == 2 && indexPath.row == 0
             ? MAX(240, self.view.bounds.size.height * 0.55)
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (check || external) {
    NSString *key =
        external ? @"VIEW_RELEASE"
                 : (fetchingLatestVersion
                        ? @"UPDATES_CHECKING"
                        : (lastChecked ? (releaseFetchFailed || !latestVersion
                                              ? @"NOTES_RETRY"
                                              : @"CHECK_AGAIN")
                                       : @"CHECK_UPDATES"));
    cell.textLabel.text = [self localized:key];
    cell.imageView.image = SHDWSettingsSymbol(
        external ? @"arrow.up.right.square" : @"arrow.clockwise");
    ...
    cell.accessibilityTraits =
        ...
        (fetchingLatestVersion ? UIAccessibilityTraitNotEnabled : 0);
  } else if (indexPath.section == 0) {
    cell.textLabel.text = [self localized:@"INSTALLED_VERSION"];
    cell.detailTextLabel.text =
        SHDWInstalledVersion() ?: [self localized:@"UNKNOWN"];
  } else if (indexPath.section == 1) {
    cell.textLabel.text = [self
        localized:@[ @"LATEST_VERSION", @"UPDATE_STATUS",
                     @"LAST_CHECKED" ][indexPath.row]];
    if (indexPath.row == 0)
      cell.detailTextLabel.text = latestVersion ?: [self localized:@"UNKNOWN"];
    else if (indexPath.row == 1)
      cell.detailTextLabel.text = [self updateStatus];
    else
      cell.detailTextLabel.text =
          [NSDateFormatter localizedStringFromDate:lastChecked
                                         dateStyle:NSDateFormatterMediumStyle
                                         timeStyle:NSDateFormatterShortStyle];
  } else {
    cell.isAccessibilityElement = NO;
    UITextView *text =
        [[UITextView alloc] initWithFrame:cell.contentView.bounds];
    ...
    text.textColor = [UIColor respondsToSelector:@selector(labelColor)]
                         ? [UIColor labelColor]
                         : [UIColor blackColor];
    ...
    NSString *body = [latestBody
        stringByTrimmingCharactersInSet:[NSCharacterSet
                                            whitespaceAndNewlineCharacterSet]];
    text.text = [NSString
        stringWithFormat:@"%@\n\n%@", latestTitle,
                         body.length ? latestBody
                                     : [self localized:@"NOTES_EMPTY"]];
...
- (void)tableView:(UITableView *)tableView
  ...
  else if (indexPath.section == 2 && indexPath.row == 1 && releaseURL &&
           !fetchingLatestVersion) {
