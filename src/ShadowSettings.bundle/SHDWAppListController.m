...
- (NSArray *)specifiers {
  if (!_specifiers) {
    ...
    self.title = [bundle localizedStringForKey:@"APP_SETTINGS"
                                         value:nil
                                         table:@"App"];
    ...
    LSApplicationProxy *proxy =
        [LSApplicationProxy applicationProxyForIdentifier:[self applicationID]];
...
- (void)updateSettingsGroupFooter {
  ...
  NSString *footer =
      [self followGlobal]
          ? [bundle
                localizedStringForKey:@"APP_USES_GLOBAL"
                                value:
                                    @"Following the global settings. Turn off to customize this app."
                                table:@"App"]
          : [bundle
                localizedStringForKey:@"APP_SETTINGS_DESC"
                                value:
                                    @"Enable Shadow for this application. Shadow uses its built-in bypass profile."
                                table:@"App"];
...
- (void)setPreferenceValue:(id)value forSpecifier:(PSSpecifier *)specifier {
  ...
  if ([key isEqualToString:@"App_FollowGlobal"]) {
    ...
    if ([value boolValue]) {
      ...
      SHDWWriteAppEnabled(prefs, [self applicationID],
                          SHDWAppEnabled(prefs, [self applicationID]));
      SHDWWriteAppAggressive(prefs, [self applicationID],
                             SHDWAppAggressive(prefs, [self applicationID]));
      // App_Enabled sits under the activation group (after the toggle);
      // Detector_Aggressive sits under its own group header.
      [self insertSpecifier:enabledSpecifier
             afterSpecifier:[self specifierForID:@"App_FollowGlobal"]
                   animated:YES];
      [self insertSpecifier:aggressiveGroupSpecifier
             afterSpecifier:enabledSpecifier
                   animated:YES];
      [self insertSpecifier:aggressiveSpecifier
             afterSpecifier:aggressiveGroupSpecifier
                   animated:YES];
...
- (void)resetAppSettings:(id)sender {
  NSBundle *bundle = [NSBundle bundleForClass:[self class]];
  NSString *title = [bundle localizedStringForKey:@"RESET_APP"
                                            value:nil
                                            table:@"App"];
  UIAlertController *alert = [UIAlertController
      alertControllerWithTitle:title
                       message:[bundle
                                   localizedStringForKey:@"RESET_APP_CONFIRM"
                                                   value:nil
                                                   table:@"App"]
                preferredStyle:UIAlertControllerStyleAlert];
  [alert addAction:[UIAlertAction
                       actionWithTitle:[bundle
                                           localizedStringForKey:@"RESET_CANCEL"
                                                           value:nil
                                                           table:@"App"]
                        ...
                        if (saved) {
                          ...
                          UIAlertController *failure = [UIAlertController
                              alertControllerWithTitle:
                                  [bundle
                                      localizedStringForKey:@"RESET_APP_FAILED"
                                                      value:nil
                                                      table:@"App"]
                                               message:
                                                   [bundle
                                                       localizedStringForKey:
                                                           @"RESET_APP_FAILED_DESC"
                                                                       value:nil
                                                                       table:
                                                                           @"App"]
                                        preferredStyle:
                          ...
                          [failure
                              addAction:
                                  [UIAlertAction
                                      actionWithTitle:
                                          [bundle
                                              localizedStringForKey:@"RESET_OK"
                                                              value:nil
                                                              table:@"App"]
