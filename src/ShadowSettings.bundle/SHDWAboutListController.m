...
- (NSString *)aboutTranslator:(id)sender {
  return [[NSBundle bundleForClass:[self class]]
      localizedStringForKey:@"TRANSLATOR"
                      value:@"Unknown"
                      table:@"About"];
}
...
- (void)openGitHub:(id)sender {
  [self
      openExternalURL:[NSURL
                          URLWithString:@"https://github.com/jjolano/shadow"]];
}
...
- (void)openSponsors:(id)sender {
  [self openExternalURL:
            [NSURL URLWithString:@"https://github.com/sponsors/jjolano"]];
}
...
- (void)resetSettings:(id)sender {
  ...
  UIAlertController *alert = [UIAlertController
      alertControllerWithTitle:[self localized:@"RESET_SETTINGS"]
                       message:[self localized:@"RESET_CONFIRM"]
                preferredStyle:UIAlertControllerStyleAlert];

  [alert
      addAction:[UIAlertAction actionWithTitle:[self localized:@"RESET_CANCEL"]
                                         style:UIAlertActionStyleCancel
                                       handler:nil]];
  [alert addAction:[UIAlertAction
                       actionWithTitle:[self localized:@"RESET_SETTINGS"]
                                 style:UIAlertActionStyleDestructive
                               handler:^(UIAlertAction *action) {
                                 [[ShadowSettings sharedInstance] reset];
                                 // Match the switch-flip feedback used across
                                 // the panes so the wipe registers as a
                                 // completed state change, not a silent no-op.
                                 SHDWToggleHaptic();
                               }]];
