...
static double shdw_dpkg_info_mtime(void) {
  double latest = 0;
  for (NSString *p in
       @[ JBPath(@"/Library/dpkg/info"), JBPath(@"/var/lib/dpkg/info") ]) {
    NSDictionary *attrs =
        [[NSFileManager defaultManager] attributesOfItemAtPath:p error:nil];
    double m =
...
static void setup_watcher(void) {
  ...
  void (^onChange)(void) = ^{
    uint64_t seen =
        atomic_fetch_add_explicit(&generation, 1, memory_order_relaxed) + 1;

    dispatch_after(
        dispatch_time(DISPATCH_TIME_NOW, kWatcherDebounceNs), queue, ^{
          if (atomic_load_explicit(&generation, memory_order_relaxed) == seen) {
            ...
            if (result == 1) {
              ...
              fprintf(stderr,
                      "error: failed to regenerate installed-apps ruleset\n");
            }
  ...
  notify_register_dispatch("com.apple.mobile.application_installed",
                           &notify_tokens[0], queue, ^(int token) {
                             (void)token;
                             onChange();
                           });
  notify_register_dispatch("com.apple.mobile.application_uninstalled",
                           &notify_tokens[1], queue, ^(int token) {
                             (void)token;
                             onChange();
                           });

  fprintf(stderr,
          "shadow-watcher: watching app install/uninstall notifications\n");
}
...
int main(int argc, char *argv[]) {
  @autoreleasepool {
    if (argc == 1) {
      ...
      printf(
          "\t-g: regenerate dpkg ruleset; system ruleset regenerated only when iOS version or snapshot changed\n");
      printf(
          "\t-d: watcher daemon - regenerate installed-apps ruleset on app install/uninstall\n");
