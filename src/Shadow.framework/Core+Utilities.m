...
+ (NSString *)getStandardizedPath:(NSString *)path {
  ...
  if ([path hasPrefix:@"/"] && ![path hasSuffix:@"/"] &&
    ...
    if (ascii &&
        strspn(
            ...
            path.length) {
      if ([path isEqualToString:@"/private/var"] ||
          [path hasPrefix:@"/private/var/"] ||
          [path isEqualToString:@"/private/etc"] ||
          [path hasPrefix:@"/private/etc/"]) {
  ...
  // ponytail: /./ and // collapse are kept — NSURL standardizedURL preserves
  // empty path segments.
  while ([path containsString:@"//"]) {
    path = [path stringByReplacingOccurrencesOfString:@"//" withString:@"/"];
  }
...
// code from Choicy
// methods of getting executablePath and bundleIdentifier with the least side
// effects possible for more information, check out
// https://github.com/checkra1n/BugTracker/issues/343
...
+ (NSString *)getBundleIdentifier {
  ...
  return mainBundle ? (__bridge NSString *)CFBundleGetIdentifier(mainBundle)
                    : nil;
}

+ (NSArray *)filterPathArray:(NSArray *)array
                  restricted:(BOOL)restricted
                     options:(NSDictionary<NSString *, id> *)options {
  Shadow *shadow = [Shadow sharedInstance];
  __block BOOL _restricted = restricted;

  NSIndexSet *indexes = [array
...
+ (NSError *)fileErrorWithCode:(NSInteger)code
                          path:(NSString *)path
                           url:(NSURL *)url {
...
+ (NSError *)fileNoSuchFileErrorForURL:(NSURL *)url {
  return [self fileErrorWithCode:NSFileNoSuchFileError
                            path:url ? [url path] : nil
                             url:url];
}
@end
