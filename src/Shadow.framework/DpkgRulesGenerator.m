...
+ (NSDictionary *)_generateDpkgRuleset {
  ...
  SHADOW_INTERNAL_SCOPE {
    ...
    NSSet *skippedLists =
        [NSSet setWithObjects:@"base.list", @"firmware-sbin.list", nil];
    ...
    NSArray *files = [[NSFileManager defaultManager]
          contentsOfDirectoryAtURL:[NSURL fileURLWithPath:dpkgInfoPath
                                              isDirectory:YES]
    ...
    for (NSURL *file in files) {
      ...
      @autoreleasepool {
        NSString *content =
            [NSString stringWithContentsOfURL:file
        ...
        for (NSString *line in
          ...
          if ([[path pathExtension] isEqualToString:@"app"]) {
            NSDictionary *plist =
                [[NSBundle bundleWithPath:JBPath(path)] infoDictionary];

            if (!IsShadowVerificationBundle(
                    [plist objectForKey:@"CFBundleIdentifier"])) {
              for (NSDictionary *type in
                   [plist objectForKey:@"CFBundleURLTypes"]) {
          ...
          [([skippedLists containsObject:[file lastPathComponent]]
                ? exceptions
                : installed) addObject:path];
        }
      }
    }
    ...
    NSPredicate *emoji = [NSPredicate
        predicateWithFormat:
            @"SELF LIKE '/System/Library/PrivateFrameworks/CoreEmoji.framework/*.lproj'"];
    [installed filterUsingPredicate:[NSCompoundPredicate
                                        notPredicateWithSubpredicate:emoji]];
    ...
    database = @{
      @"RulesetInfo" :
          @{@"Name" : @"dpkg installed files", @"Author" : @"Shadow Service"},
      @"BlacklistExactPaths" :
          [[installed allObjects] sortedArrayUsingSelector:@selector(compare:)],
      @"BlacklistURLSchemes" :
          [[schemes allObjects] sortedArrayUsingSelector:@selector(compare:)]
    };
  }
