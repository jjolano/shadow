...
- (double)_fileMtime:(NSString *)path {
  ...
  // ns mtime: iOS gives ns; host harness may be 1s-granularity but we still
  // store ns
...
- (ShadowRulesetSnapshot *)_loadSnapshot {
  ...
  SHADOW_INTERNAL_SCOPE {
    ...
    NSMutableDictionary<NSString *, NSNumber *> *mtimes =
    ...
    NSArray *ruleset_urls =
        [fm contentsOfDirectoryAtURL:[NSURL fileURLWithPath:dir isDirectory:YES]
            includingPropertiesForKeys:@[]
                               options:0
                                 error:nil];

    if (ruleset_urls) {
      // Sort by file name so both load order and the mtime snapshot are
      // deterministic.
      ruleset_urls = [ruleset_urls
      ...
      // prune stale .shadowcache: ruleset .shadowcache without matching plist
      // (ns check via compiler)
      for (NSURL *cUrl in ruleset_urls) {
        if ([[cUrl lastPathComponent] hasSuffix:kShadowRulesetCacheSuffix]) {
          NSString *plistPath = [[cUrl path]
              stringByReplacingOccurrencesOfString:kShadowRulesetCacheSuffix
                                        withString:@""];
          if (![[NSFileManager defaultManager] fileExistsAtPath:plistPath]) {
            @try {
              [[NSFileManager defaultManager] removeItemAtPath:[cUrl path]
                                                         error:nil];
            } @catch (NSException *e) {
            }
          }
      ...
      for (NSURL *url in ruleset_urls) {
        ...
        if (ruleset) {
          ...
          if (info) {
            NSLog(@"[Shadow] loaded ruleset: '%@' by %@ (%@)",
                  [info objectForKey:@"Name"], [info objectForKey:@"Author"],
                  url);
          } else {
        ...
        // Snapshot every file in the dir (all plists load; a stray non-plist is
        // still tracked so its rewrite is caught).
    ...
    atomic_store_explicit(&lastRulesetCheck,
                          [NSDate timeIntervalSinceReferenceDate],
                          memory_order_release);
    snapshot = [ShadowRulesetSnapshot snapshotWithRulesets:result
...
- (void)_reloadRulesets {
  ...
  if ([fresh.rulesets count] == 0 && [previous.rulesets count] > 0) {
    NSLog(
        @"[Shadow] rulesets reload produced an empty set; keeping last-known-good snapshot");
    return;
  }
  ...
  @synchronized(self) {
    _generation += 1;
    current = [ShadowRulesetSnapshot snapshotWithRulesets:fresh.rulesets
                                               generation:_generation];
  }
  ...
  atomic_store_explicit(&shdw_ruleset_generation, (uint64_t)_generation,
                        memory_order_release);
}
...
- (void)checkForChanges {
  ...
  SHADOW_INTERNAL_SCOPE {
    ...
    if (!atomic_compare_exchange_strong_explicit(&lastRulesetCheck, &expected,
