...
extern int fs_snapshot_list(int fd, struct attrlist *alist, void *buf,
                            size_t bufsize, uint32_t flags)
    __attribute__((weak_import));
extern int fs_snapshot_mount(int fd, const char *dir, const char *name,
...
// /usr covers /usr/lib, /usr/bin, /usr/sbin, /usr/libexec and /usr/share at
// full depth.
...
static const NSUInteger kSystemZoneCount =
    sizeof(kSystemZones) / sizeof(kSystemZones[0]);
  ...
  dispatch_once(&onceToken, ^{
    ...
    stock = @{
      ...
      @"/System" : [NSSet setWithArray:@[
        @"Library", @"Applications", @"iOSSupport", @"Cryptexes", @"Developer",
        @"DriverKit"
      ]],
      @"/System/Library" : [NSSet setWithArray:@[
        ...
        @"AppRemovalServices", @"AppSignatures", @"AssetTypeDescriptors",
        @"Assistant", @"Audio", @"AWD", @"BackBoard", @"Backup",
        @"BridgeManifests", @"BulletinDistributor", @"CacheDelete", @"Caches",
        @"CardKit", @"CardServices", @"Carrier Bundles", @"ColorPickers",
        @"ColorSync", @"Components", @"CompositeServices", @"ControlCenter",
        @"CoreAccessories", @"CoreAS", @"CoreDuet", @"CoreImage",
        @"CoreServices", @"CountryBundles", @"CryptoTokenKit", @"DataAccess",
        @"DataClassMigrators", @"DefaultsConfigurations", @"Developer",
        @"DeviceOMatic", @"DifferentialPrivacy", @"DistributedEvaluation",
        @"DoNotDisturb", @"DriverExtensions", @"DuetActivityScheduler",
        @"DuetExpertCenter", @"DuetKnowledgeBase", @"Extensions", @"FDR",
        @"FeatureFlags", @"Filesystems", @"Fitness", @"Fonts", @"fps",
        @"Frameworks", @"Health", @"HIDPlugins", @"IdentityServices",
        @"Internet Plug-Ins", @"Jet", @"KerberosPlugins", @"Keyboard",
        @"KeyboardLayouts", @"KeyboardParameters", @"Keyboards",
        @"LaunchAgents", @"LaunchDaemons", @"Lexicons", @"LifecyclePolicy",
        @"LinguisticData", @"LocationBundles", @"Lockdown", @"Logging",
        @"Media", @"MediaCapture", @"MediaStreamPlugins", @"Messages",
        @"Migration", @"NanoLaunchDaemons", @"NanoLaunchDaemonsAltAccount",
        @"NanoPreferenceBundles", @"NanoTimeKit", @"NetworkServiceProxy",
        @"Obliteration", @"OnBoardingBundles", @"PairedSyncServices",
        @"PerfPowerTelemetry", @"Photos", @"PreferenceBundles",
        @"PreferenceManifests", @"PreferenceManifestsInternal", @"Preferences",
        @"PreferencesSyncBundles", @"PreinstalledAssets",
        @"PreinstalledAssetsV2", @"Previews", @"Printers", @"PrivateFrameworks",
        @"ProceduralWallpaper", @"ProductDocuments", @"QuickLook", @"Recents",
        @"RelevanceEngine", @"RunningBoard", @"Security", @"SESStorage",
        @"Sandbox", @"ScreenReader", @"SetupAssistantBundles", @"SmsFilter",
        @"Snippets", @"SoftwareUpdateCertificates", @"Sounds", @"Spotlight",
        @"SpringBoardPlugins", @"SyncBundles", @"System",
        @"SystemConfiguration",
        ...
        @"UserNotifications", @"Video", @"VideoCodecs", @"VideoDecoders",
        @"VideoEncoders", @"VideoProcessors", @"VoiceServices", @"Wallpaper",
        @"Watch", @"WRM", @"xpc"
      ]]
    };
  });
...
// Cryptex volumes (/System/Cryptexes and children) are sealed, read-only,
// signed IMG4 mounts: never part of the system APFS snapshot, and jailbreak
// files can never appear in them. They must always be read from the live
// filesystem.
static BOOL IsCryptexZone(NSString *zonePath) {
  return [zonePath isEqualToString:@"/System/Cryptexes"] ||
         [zonePath hasPrefix:@"/System/Cryptexes/"];
}
...
+ (BOOL)_buildStructure:(NSMutableDictionary *)structure
                 fsPath:(NSString *)fsPath
              canonPath:(NSString *)canonPath
  ...
  for (NSString *entry in entries) {
    ...
    if (descend) {
      NSString *fileType = [[fm attributesOfItemAtPath:entryPath error:NULL]
          objectForKey:NSFileType];

      if ([fileType isEqualToString:NSFileTypeDirectory]) {
        [self _buildStructure:structure
                       fsPath:entryPath
                    canonPath:[canonPath stringByAppendingPathComponent:entry]
...
+ (BOOL)_forEachSystemZone:(NSString *)fsPrefix
               cryptexLive:(BOOL)cryptexLive
                     block:(BOOL (^)(NSString *zonePath, NSString *fsPath,
                                     const SystemZone *zone))block {
  for (NSUInteger i = 0; i < kSystemZoneCount; i++) {
    ...
    NSString *fsPath = (fsPrefix && !isCryptex)
                           ? [fsPrefix stringByAppendingPathComponent:zonePath]
                           : zonePath;
...
+ (BOOL)_walkStructureZonesWithPrefix:(NSString *)fsPrefix
                                 into:(NSMutableDictionary *)structure {
  return [self
      _forEachSystemZone:fsPrefix
             cryptexLive:YES
                   block:^BOOL(NSString *zonePath, NSString *fsPath,
                               const SystemZone *zone) {
                     if (![self _buildStructure:structure
                                         fsPath:fsPath
                                      canonPath:zonePath
                                      remaining:zone->depth]) {
                       fprintf(
                           stderr,
                           "warning: SystemRules: failed walking %s, keeping partial structure\n",
                           zone->path);
                       return NO;
                     }
...
// Collects every entry (files, dirs, symlinks) under fsPath, canonicalized,
// full depth.
+ (void)_collectPaths:(NSString *)fsPath
            canonPath:(NSString *)canonPath
                 into:(NSMutableSet *)set {
  NSFileManager *fm = [NSFileManager defaultManager];
  NSArray *entries = [fm contentsOfDirectoryAtPath:fsPath error:NULL];
  ...
  for (NSString *entry in entries) {
    NSString *canon = [Shadow
        getStandardizedPath:[canonPath stringByAppendingPathComponent:entry]];
...
+ (void)_collectZonePathsWithPrefix:(NSString *)fsPrefix
                               into:(NSMutableSet *)set {
  [self _forEachSystemZone:fsPrefix
               cryptexLive:NO
                     block:^BOOL(NSString *zonePath, NSString *fsPath,
                                 const SystemZone *zone) {
                       (void)zone;
...
+ (void)_diffWalk:(NSString *)fsPath
        canonPath:(NSString *)canonPath
      snapshotSet:(NSSet *)snapshotSet
            exact:(NSMutableArray *)exact
             dirs:(NSMutableArray *)dirs {
  NSFileManager *fm = [NSFileManager defaultManager];
  NSArray *entries = [fm contentsOfDirectoryAtPath:fsPath error:NULL];
...
+ (void)_diffZonesWithPrefix:(NSString *)fsPrefix
                 snapshotSet:(NSSet *)snapshotSet
                       exact:(NSMutableArray *)exact
                        dirs:(NSMutableArray *)dirs {
  [self _forEachSystemZone:fsPrefix
...
+ (NSString *)_findSnapshotNameWithFd:(int *)outFd {
  ...
  const uint32_t kNameRefOffset =
      (uint32_t)(sizeof(uint32_t) + sizeof(attribute_set_t));
  const uint32_t kMinRecord =
      kNameRefOffset + (uint32_t)sizeof(attrreference_t) + 1;

  uint32_t recordCount =
      (ret < (int32_t)kMaxRecords) ? (uint32_t)ret : kMaxRecords;
  ...
  for (uint32_t record = 0; record < recordCount; record++) {
    ...
    NSString *name = [[NSString alloc] initWithBytes:nameStr
                                              length:nameLen
...
+ (NSDictionary *)_currentRulesetIdentity {
  NSMutableDictionary *identity =
      [NSMutableDictionary dictionaryWithObject:[self _currentiOSVersion]
                                         forKey:@"iOSVersion"];
...
+ (NSDictionary *)generateSystemRuleset {
  ...
  if (!JBIsRootless()) {
    ...
    if (snapshotName && fd >= 0) {
      ...
      BOOL mounted = fs_snapshot_mount &&
                     (fs_snapshot_mount(fd, [mountpoint UTF8String],
                                        [snapshotName UTF8String], 0) == 0);

      if (mounted) {
        snapshotUsed = YES;
        snapshotNameUsed = snapshotName;

        // Structure comes from the stock snapshot; a partial walk falls back to
        // live below.
        snapshotWalkOK = [self _walkStructureZonesWithPrefix:mountpoint
                                                        into:structure];
        ...
        if ([snapshotSet count] > 0) {
          ...
          fprintf(
              stderr,
              "warning: SystemRules: snapshot walk returned nothing, skipping diff\n");
        }
        ...
        fprintf(
            stderr,
            "warning: SystemRules: failed to mount snapshot, using live filesystem\n");
      }
      ...
      fprintf(
          stderr,
          "warning: SystemRules: no usable system snapshot, using live filesystem\n");
    }
  }
  ...
  if (snapshotUsed && !snapshotWalkOK) {
    fprintf(
        stderr,
        "warning: SystemRules: partial snapshot walk, falling back to live filesystem\n");
    snapshotUsed = NO;
  }
  ...
  NSMutableDictionary *ruleset_info =
  ...
  [ruleset setObject:@{
    @"dirs" :
        [[dirSet allObjects] sortedArrayUsingSelector:@selector(compare:)],
    @"paths" :
        [[pathSet allObjects] sortedArrayUsingSelector:@selector(compare:)]
  }
              forKey:@"FileSystemStructure"];

  if ([exact count] > 0 || [dirs count] > 0) {
    // Overlapping zones (/System vs /System/Library) produce duplicates; dedupe
    // before the cap.
...
+ (NSInteger)writeSystemRuleset {
  ...
  if (previous && [previousVersion isEqualToString:currentVersion] &&
      (!currentSnapshot ||
       [previousSnapshot isEqualToString:currentSnapshot])) {
  ...
  NSString *currentSnapshotUsed =
      [[ruleset objectForKey:@"RulesetInfo"] objectForKey:@"SnapshotName"];

  if (previousSnapshot && !currentSnapshotUsed) {
    fprintf(
        stderr,
        "warning: SystemRules: no system snapshot available (previously '%s'); using live filesystem\n",
        [previousSnapshot UTF8String]);
  } else if (previousSnapshot && currentSnapshotUsed &&
             ![previousSnapshot isEqualToString:currentSnapshotUsed]) {
    fprintf(
        stderr,
        "note: SystemRules: system snapshot changed '%s' -> '%s' (iOS update?); regenerated against the current snapshot\n",
...
+ (NSDictionary *)generateInstalledAppsRuleset {
  ...
  NSArray *urls = [[NSFileManager defaultManager]
        contentsOfDirectoryAtURL:[NSURL fileURLWithPath:dir isDirectory:YES]
  ...
  for (NSURL *url in urls) {
    ...
    double mtime =
        attrs ? [[attrs fileModificationDate] timeIntervalSinceReferenceDate]
              : 0.0;
    ...
    NSDictionary *info =
        [[ruleset payloadDictionary] objectForKey:@"RulesetInfo"];

    if ([[[info objectForKey:@"Author"] lowercaseString]
            isEqualToString:@"shadow service"]) {
  ...
  for (LSApplicationProxy *proxy in [workspace allInstalledApplications]) {
    ...
    if (!restricted) {
      ...
      if (bundlePath &&
          shdw_is_restricted_root([bundlePath fileSystemRepresentation])) {
        restricted = YES;
      }
    }
    ...
    NSDictionary *info =
        [[NSBundle bundleWithPath:[[proxy bundleURL] path]] infoDictionary];
  ...
  NSMutableDictionary *ruleset_dict =
      [NSMutableDictionary dictionaryWithDictionary:@{
        @"RulesetInfo" : @{
  ...
  if ([schemes count] > 0) {
    [ruleset_dict setObject:[[schemes allObjects]
                                sortedArrayUsingSelector:@selector(compare:)]
                     forKey:@"BlacklistURLSchemes"];
  }

  if ([bundleids count] > 0) {
    [ruleset_dict setObject:[[bundleids allObjects]
                                sortedArrayUsingSelector:@selector(compare:)]
