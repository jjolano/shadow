...
shdwInstallHarnessSDKFallback(void) {
...
+ (NSString *)shdwMCMContainerPathForBundleID:(NSString *)bid
  ...
  if (!bid || !dataRoot)
    return nil;
  NSArray *children =
      [[NSFileManager defaultManager] contentsOfDirectoryAtPath:dataRoot
                                                          error:nil];
  NSString *matched = nil;
  NSUInteger matchCount = 0;
  for (NSString *child in children) {
    NSString *meta = [[dataRoot stringByAppendingPathComponent:child]
...
- (instancetype)init {
  if ((self = [super init])) {
    bundlePath =
        [[[self class] getExecutablePath] stringByDeletingLastPathComponent];
    ...
    if (!hasAppSandbox) {
      ...
      if (bid) {
        SHADOW_INTERNAL_SCOPE {
          NSString *mcmPath = [[self class
          ] shdwMCMContainerPathForBundleID:bid
                                   dataRoot:
    ...
    SHADOW_INTERNAL_SCOPE {
      NSArray *found =
          [[self class] shdwGroupContainersUnderRoot:
...
- (BOOL)isMountPathRestricted:(const char *)path {
  ...
  @autoreleasepool {
    @try {
      SHADOW_INTERNAL_SCOPE {
        restricted =
            [engine isMountPathRestricted:[NSString stringWithUTF8String:path]];
      }
...
static ShadowRestrictionQuery *
shdwQueryFromOptions(NSString *path, NSDictionary<NSString *, id> *options) {
  ...
  if ([[options objectForKey:kShadowRestrictionOperation]
          isEqualToString:kShadowRestrictionOpWrite]) {
...
- (BOOL)isPathRestrictedQuery:(ShadowRestrictionQuery *)query {
  ...
  @try {
    SHADOW_INTERNAL_SCOPE { restricted = [engine isPathRestrictedQuery:query]; }
  } @catch (NSException *exception) {
  ...
  @autoreleasepool {
    ...
    dispatch_once(&onceToken, ^{
      staticBundleIDs = [NSSet setWithArray:@[
        ...
        @"jp.r333d.taurine", @"com.undecimus.unc0ver", @"eu.taurine.taurine",
        @"com.apt.theos"
      ]];
    });
  ...
  @autoreleasepool {
    ...
    dispatch_once(&onceToken, ^{
      protectedNames = [NSSet setWithArray:@[
        @"shadow.dylib", @"shadowcore", @"shadow.framework", @"libsandy.dylib",
        @"hookkit.framework", @"substrate", @"libsubstrate", @"substitute",
        @"libsubstitute", @"ellekit", @"libellekit"
      ]];
    });
...
- (NSString *)description {
  return
      [NSString stringWithFormat:
                    @"<ShadowRestrictionQuery path=%@ wd=%@ op=%ld flags=%lu>",
                    self.path, self.workingDirectory, (long)self.operation,
