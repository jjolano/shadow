...
- (BOOL)path:(NSString *)path hasFilenamePrefix:(NSString *)prefix {
  ...
  // Prefix may end mid-filename (com.apple -> com.apple.locationd.plist), but
  // must not span a slash boundary (com.apple -> com.appleEvil/subdir/file is a
  // miss). Search from prefix_len + 1: the boundary char itself may be '/'.
  return [path hasPrefix:prefix] &&
         [path rangeOfString:@"/"
                     options:0
                       range:NSMakeRange(prefix_len + 1,
                                         [path length] - prefix_len - 1)]
                 .location == NSNotFound;
}
...
- (BOOL)_path:(NSString *)path
    matchesPrefixDict:(NSDictionary<NSString *, NSSet<NSString *> *> *)dict
  ...
  return [[dict objectForKey:[parent stringByDeletingLastPathComponent]]
      containsObject:parent];
}
...
- (BOOL)_structureContains:(NSArray *)array path:(NSString *)path {
  if (!array || !path)
    return NO;
  NSUInteger idx = [array indexOfObject:path
  ...
  while (tmp.length > 0) {
    if ([self _structureContains:dirs path:tmp]) {
      ...
      NSRange slash =
          [path rangeOfString:@"/"
                      options:0
                        range:NSMakeRange(base, [path length] - base)];
      NSString *component =
          (slash.location == NSNotFound)
              ? [path substringFromIndex:base]
              : [path substringWithRange:NSMakeRange(base,
                                                     slash.location - base)];

      return [self
          _structureContains:paths
                        path:[tmp stringByAppendingPathComponent:component]];
    }
...
- (BOOL)isPathWhitelisted:(NSString *)path {
  return [self _path:path
          matchesSet:set_whitelist
          prefixDict:dict_whitelist
            matchAll:whitelist_match_all
           predicate:pred_whitelist];
}

- (BOOL)isPathBlacklisted:(NSString *)path {
  return [self _path:path
          matchesSet:set_blacklist
          prefixDict:dict_blacklist
            matchAll:blacklist_match_all
           predicate:pred_blacklist];
}
...
- (BOOL)_path:(NSString *)path
    matchesSet:(NSSet *)set
    prefixDict:(NSDictionary *)dict
      matchAll:(BOOL)matchAll
