...
| `me.jjolano.shadow_<ver>_iphoneos-arm.deb` (rootful: armv7/armv7s + arm64/arm64e) | `root/` |
...
- **The rootful deb replaces both the old rooted and legacy packages.** `control` keeps `Conflicts: me.jjolano.shadow.legacy`, so installing it on a device with the old v2-era `me.jjolano.shadow.legacy` package auto-removes it (dpkg deinstalls conflicted packages). Never ship the old legacy id — the rootful deb is the single `iphoneos-arm` entry.
...
1. Build: `./build.sh all` → rootless + rootful + roothide debs in `build/`
