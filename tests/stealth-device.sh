...
task_scopes = {
    'ORA-02': ('[private-harness]/', 'ShadowCore.dylib/dylib.x',
    ...
    'ORA-03': ('tools/dyldprobe/', 'Shadow.dylib/dylib.x',
               ...
               'tests/stealth-device.sh', 'tests/stealth_validate.py'),
    'ORA-04': ('[private-harness]/', 'tools/dyldprobe/', 'Shadow.dylib/dylib.x',
               'ShadowCore.dylib/hooks/Runtime/dyld.x', 'Shadow.framework/Core.m',
               'tests/stealth-device.sh', 'tests/stealth_validate.py'),
}
...
emit_file harness me.jjolano.shadow.harness /var/jb/Applications/[private-harness].app/[private-harness] false
...
PY
    ...
    case "$comm" in
        /var/jb/usr/bin/hookprobe|/usr/bin/hookprobe|*/[private-harness].app/[private-harness]|*/dyldprobe.app/dyldprobe) ;;
...
find_local_recovery_deb() {
    ...
    while IFS= read -r -d '' candidate; do
        ...
        rm -rf "$tmp"
    done < <(find "$REPO_ROOT/build" "$REPO_ROOT/packages" "$REPO_ROOT/[private-harness]/packages" "$REPO_ROOT/tools/dyldprobe/build" "$REPO_ROOT/artifacts/stealth" -type f -name '*.deb' -print0 2>/dev/null)
    rm -rf "$cache"
...
bundle_executable() {
    case "$1" in
        me.jjolano.shadow.harness) printf /var/jb/Applications/[private-harness].app/[private-harness] ;;
...
emit_component harness /var/jb/Applications/[private-harness].app/[private-harness]
