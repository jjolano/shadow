...
const char *SHDWFreeRASPGenericProbeJSON(void) {
    ...
    dlerror();
    void *rootHidden = dlopen("@executable_path/.jbroot/libRH_FP_0_0.dat", RTLD_NOW);
    const char *rootHiddenError = rootHidden ? NULL : dlerror();
    unsigned rootHiddenQuotes = 0;
    if(rootHiddenError) {
        for(const char *quote = rootHiddenError; (quote = strchr(quote, '\'')); quote++) rootHiddenQuotes++;
    }
    int rootHiddenCryptex = rootHiddenError && strstr(rootHiddenError, "Cryptexes/OS@") != NULL;
    int rootHiddenExecutable = rootHiddenError && strstr(rootHiddenError, "OS@executable_path") != NULL;
    int rootHiddenLoader = rootHiddenError && strstr(rootHiddenError, "OS@loader_path") != NULL;
    int rootHiddenRpath = rootHiddenError && strstr(rootHiddenError, "OS@rpath") != NULL;
    int rootHiddenMatch = rootHiddenCryptex &&
        (rootHiddenExecutable || rootHiddenLoader || rootHiddenRpath || rootHiddenQuotes > 2);
    if(rootHidden) dlclose(rootHidden);
    ...
    snprintf(json, sizeof(json),
        ...
        "\"rootHidden\":{\"loaded\":%d,\"match\":%d,\"cryptex\":%d,\"executable\":%d,\"loader\":%d,\"rpath\":%d,\"quotes\":%u},"
        ...
        rootHidden != NULL, rootHiddenMatch, rootHiddenCryptex, rootHiddenExecutable,
        rootHiddenLoader, rootHiddenRpath, rootHiddenQuotes,
