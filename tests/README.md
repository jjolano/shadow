...
Use `tests/tools/dyldprobe`, `tests/tools/hookprobe`, `tests/[private-harness]`, and
...
```sh
make -C tests harness-regression DEVICE=[device-address]
make -C tests harness-regression DEVICE=[device-address] DEB=../build/me.jjolano.shadow.harness_1.0.0_iphoneos-arm64.deb
python3 tests/harness-regression.py --device [device-address] --update   # rebaseline after a verified change
```
