...
if grep -Rqs 'shdw_freerasp_start_disabled\|kSHDWFreeRASPStartSymbol' ShadowCore.dylib/hooks ||
   ...
   ! grep -q '"talsecStartReturned": talsecStartReturned' [private-runner]/FreeRASP/AppDelegate.swift ||
   ! grep -q '"talsecImageLoaded": talsecImageBase() != nil' [private-runner]/FreeRASP/AppDelegate.swift ||
   ! grep -q '"talsecAllChecksFinished": ThreatStore.shared.finished()' [private-runner]/FreeRASP/AppDelegate.swift ||
...
if ! grep -q '\[shdw_coordinator_instance prearmDetector\]' ShadowCore.dylib/shadowcore.x ||
   ! grep -q 'stockMarkerVisibleBeforeStart' [private-runner]/FreeRASP/AppDelegate.swift; then
