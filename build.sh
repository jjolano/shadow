...
build_harness() { # rootful-modern|rootless
    ...
    case "$lane" in
        rootful-modern) make -C [private-harness] package FINALPACKAGE=1 ARCHS='arm64 arm64e' TARGET=iphone:clang:16.5:14.0 "${MAKE_PATHS[@]}" ;;
        rootless) make -C [private-harness] package FINALPACKAGE=1 THEOS_PACKAGE_SCHEME=rootless ARCHS='arm64 arm64e' TARGET=iphone:clang:16.5:15.0 "${MAKE_PATHS[@]}" ;;
    esac
