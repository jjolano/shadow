...
build_harness() { # rootful-modern|rootless
    ...
    if [ "$lane" = rootless ]; then
        ...
        make -C tests/[private-harness] package FINALPACKAGE=1 ${scheme:+THEOS_PACKAGE_SCHEME=$scheme} \
            ARCHS="$(shadow_lane_field "$lane" ARCHS)" \
            TARGET="$(shadow_lane_field "$lane" TARGET)" "${MAKE_PATHS[@]}" ${MODERN_ARGS[@]+"${MODERN_ARGS[@]}"}
    fi
    package=$(<tests/[private-harness]/.theos/last_package)
    case "$package" in
        /*) ;;
        *) package="$ROOT/tests/[private-harness]/${package#./}" ;;
    esac
