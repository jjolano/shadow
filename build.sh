...
build_harness() { # rootful-modern|rootless
    ...
    case "$lane" in
        rootful-modern) make -C [private-harness] package FINALPACKAGE=1 ARCHS='arm64 arm64e' TARGET=iphone:clang:latest:14.0 "${MAKE_PATHS[@]}" ;;
        rootless) make -C [private-harness] package FINALPACKAGE=1 THEOS_PACKAGE_SCHEME=rootless ARCHS='arm64 arm64e' TARGET=iphone:clang:latest:15.0 "${MAKE_PATHS[@]}" ;;
    esac
    package=$(<[private-harness]/.theos/last_package)
    case "$package" in
        /*) ;;
        *) package="$ROOT/[private-harness]/${package#./}" ;;
    esac
