...
build_rootless() {
    ...
    THEOS_PACKAGE_SCHEME=rootless ARCHS="arm64 arm64e" TARGET=iphone:clang:latest:15.0 make -C [private-harness] package FINALPACKAGE=1 &&
    cp -p [private-harness]/packages/*.deb $PWD/build/
...
build_fat() {
    ...
    ARCHS="armv7 armv7s arm64 arm64e" TARGET=iphone:clang:latest:9.0 make -C [private-harness] package FINALPACKAGE=1 &&
    cp -p [private-harness]/packages/*.deb $PWD/build/
