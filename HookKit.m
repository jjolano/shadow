...
- (hookkit_status_t)hookFunction:(void *)function withReplacement:(void *)replacement outOldPtr:(void **)old_ptr {
    ...
    if(result == HK_ERR && (_types & HK_LIB_LIBHOOKER) == HK_LIB_LIBHOOKER && (handle = dlopen([libhooker_path fileSystemRepresentation], RTLD_LAZY))) {
        ...
        if(_LHHookFunctions) {
            if(_LHHookFunctions(&hook, 1) == LIBHOOKER_OK) {
                result = HK_OK;
            }
        }
    }
...
- (hookkit_status_t)hookMemory:(void *)target withData:(const void *)data size:(size_t)size {
    ...
    if(result == HK_ERR && (_types & HK_LIB_LIBHOOKER) == HK_LIB_LIBHOOKER && (handle = dlopen([libhooker_path fileSystemRepresentation], RTLD_LAZY))) {
        ...
        if(_LHPatchMemory) {
            if(_LHPatchMemory(&hook, 1) == LIBHOOKER_OK) {
                result = HK_OK;
            }
        }
    }
...
- (hookkit_status_t)performHooksWithSubstitutor:(HookKit *)substitutor {
    ...
    if((!didFunctions || !didMemory) && result == HK_ERR && ([substitutor types] & HK_LIB_LIBHOOKER) == HK_LIB_LIBHOOKER && (handle = dlopen(ROOT_PATH_C("/usr/lib/libhooker.dylib"), RTLD_LAZY))) {
        if(!didFunctions) {
            static int (*_LHHookFunctions)(const struct LHFunctionHook* hooks, int count) = NULL;
            if(!_LHHookFunctions) _LHHookFunctions = dlsym(handle, "LHHookFunctions");
            if(_LHHookFunctions) {
                ...
                if(_LHHookFunctions([hooks bytes], [functionHooks count]) == LIBHOOKER_OK) {
                    didFunctions = YES;
                }
            }
        }
        ...
        if(!didMemory) {
            ...
            if(_LHPatchMemory) {
                ...
                if(_LHPatchMemory([hooks bytes], [memoryHooks count]) == LIBHOOKER_OK) {
                    didMemory = YES;
                }
            }
        }
    ...
    if((!didFunctions || !didMemory) && result == HK_ERR && ([substitutor types] & HK_LIB_SUBSTITUTE) == HK_LIB_SUBSTITUTE && (handle = dlopen(ROOT_PATH_C("/usr/lib/libsubstitute.dylib"), RTLD_LAZY))) {
        if(!didFunctions) {
            static int (*_substitute_hook_functions)(const struct substitute_function_hook* hooks, size_t nhooks, struct substitute_function_hook_record** recordp, int options) = NULL;
            if(!_substitute_hook_functions) _substitute_hook_functions = dlsym(handle, "substitute_hook_functions");
            if(_substitute_hook_functions) {
    ...
    if((!didFunctions || !didMemory) && result == HK_ERR && ([substitutor types] & HK_LIB_SUBSTRATE) == HK_LIB_SUBSTRATE && (handle = dlopen(ROOT_PATH_C("/usr/lib/libsubstrate.dylib"), RTLD_LAZY))) {
